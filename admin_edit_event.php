<?php

/*
#######################################
#     AACGC Trophy Room               #                
#     by M@CH!N3                      #
#     http://www.AACGC.com            #
#######################################
*/




require_once("../../class2.php");
if(!getperms("P")) {
echo "";
exit;
}
require_once(e_ADMIN."auth.php");
require_once(e_HANDLER."form_handler.php"); 
require_once(e_HANDLER."file_class.php");
$rs = new form;
$fl = new e_file;
if (e_QUERY) {
        $tmp = explode('.', e_QUERY);
        $action = $tmp[0];
        $id = $tmp[1];
        unset($tmp);
}
//-----------------------------------------------------------------------------------------------------------+
if (isset($_POST['update_event'])) {
        $message = ($sql->db_Update("aacgc_trophy_room", "event_name='".$_POST['event_name']."',event_place='".$_POST['event_place']."',event_date='".$_POST['event_date']."',event_detail='".$_POST['event_detail']."' WHERE event_id='".$_POST['id']."' ")) ? "Successful updated" : "Update failed";
}

if (isset($_POST['main_delete'])) {
        $delete_id = array_keys($_POST['main_delete']);
	$sql2 = new db;
    $sql2->db_Delete("aacgc_trophy_room", "event_id='".$delete_id[0]."'");
	
}

if (isset($message)) {
        $ns->tablerender("", "<div style='text-align:center'><b>".$message."</b></div>");
}
//-----------------------------------------------------------------------------------------------------------+
if ($action == "") {
        $text .= $rs->form_open("post", e_SELF, "myform_".$row['event_id']."", "", "");
        $text .= "
        <div style='text-align:center'>
        <table style='width:95%' class='fborder' cellspacing='0' cellpadding='0'>
        <tr>
        <td style='width:' class='forumheader3'>Event ID</td>
        <td style='width:25%' class='forumheader3'>Name</td>
        <td style='width:25%' class='forumheader3'>Date</td>
        <td style='width:' class='forumheader3'>Place</td>
        <td style='width:50%' class='forumheader3'>Details</td>
        <td style='width:' class='forumheader3'>Options</td>
       </tr>";
        $sql->db_Select("aacgc_trophy_room", "*", "ORDER BY event_id ASC","");
        while($row = $sql->db_Fetch()){
        $text .= "
        <tr>
        <td style='width:' class='forumheader3'>".$row['event_id']."</td>
        <td style='width:25%' class='forumheader3'>".$row['event_name']."</td>
        <td style='width:25%' class='forumheader3'>".$row['event_date']."</td>
        <td style='width:' class='forumheader3'>".$row['event_place']."</td>
        <td style='width:50%' class='forumheader3'>".$row['event_detail']."</td>
        <td style='width:' class='forumheader3'>
        
		<a href='".e_SELF."?edit.{$row['event_id']}'>".ADMIN_EDIT_ICON."</a>
		<input type='image' title='".LAN_DELETE."' name='main_delete[".$row['event_id']."]' src='".ADMIN_DELETE_ICON_PATH."' onclick=\"return jsconfirm('".LAN_CONFIRMDEL." [ID: {$row['event_id']} ]')\"/>
		</td>
        </tr>";
		}
        $text .= "
        </table>
        </div>";
        $text .= $rs->form_close();
	      $ns -> tablerender("", $text);
	      require_once(e_ADMIN."footer.php");
}
//-----------------------------------------------------------------------------------------------------------+

//-----------------------------------------------------------------------------------------------------------+

if ($action == "edit")
{
                $sql->db_Select("aacgc_trophy_room", "*", "event_id = $id");
                $row = $sql->db_Fetch();

$options .= "
<option name='event_place' value='1'>1st</option>
<option name='event_place' value='2'>2nd</option>
<option name='event_place' value='3'>3rd</option>
";


        $width = "width:100%";
        $text = "
        <div style='text-align:center'>
        ".$rs -> form_open("post", e_SELF, "MyForm", "", "enctype='multipart/form-data'", "")."
        <table style='".$width."' class='fborder' cellspacing='0' cellpadding='0'>
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>Name:</td>
        <td style='width:70%' class='forumheader3'>
            ".$rs -> form_text("event_name", 100, $row['event_name'], 500)."
        </td>
        </tr>
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>Date:</td>
        <td style='width:70%' class='forumheader3'>
            ".$rs -> form_text("event_date", 100, $row['event_date'], 500)."
        </td>
        </tr>
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>Place:</td>
          <td style='width:70%' class='forumheader3'>
		<select name='event_place' size='1' class='tbox' style='width:100px'>
                <option name='event_place' value='".$row['event_place']."'>".$row['event_place']."</option>
		".$options."
        </td>
        </tr>
        <tr>
        <td style='width:30%; text-align:right' class='forumheader3'>Details:</td>
        <td style='width:70%' class='forumheader3'>
            ".$rs -> form_textarea("event_detail", 100, '15', $row['event_detail'], "", "", "", "", "")."
        </td>
        </tr>
";

        $text .= "</div>
        </td></tr>
        <tr style='vertical-align:top'>
        <td colspan='2' style='text-align:center' class='forumheader'>
        ".$rs->form_hidden("id", "".$row['event_id']."")."
        ".$rs -> form_button("submit", "update_event", "Update")."
        </td>
        </tr>
        </table>
        ".$rs -> form_close()."
        </div>";
	      $ns -> tablerender("", $text);
	      require_once(e_ADMIN."footer.php");
}
?>


